...
struct SafeScriptMessageHandlerTests {
    ...
    @Test func allowsPermissionGatewayFrontendOriginFromSettings() {
        let handler = SafeScriptMessageHandler(
            server: server(
                internalURL: URL(string: "http://192.168.99.10:18080")!,
                gatewayURL: URL(string: "http://192.168.99.10:18080")!,
                frontendURL: URL(string: "https://ha.aivo.19.md")!
            ),
            delegate: NoOpScriptMessageHandler()
        )

        #expect(handler.shouldAllowMessage(isMainFrame: true, scheme: "http", host: "192.168.99.10", port: 18080))
        #expect(handler.shouldAllowMessage(isMainFrame: true, scheme: "https", host: "ha.aivo.19.md", port: 443))
    }
...
private func server(internalURL: URL, gatewayURL: URL? = nil, frontendURL: URL? = nil) -> Server {
    var info = ServerInfo(
        name: "IPv6 Server",
        connection: .init(
            externalURL: nil,
    ...
    info.setSetting(value: gatewayURL?.absoluteString, for: .permissionGatewayURL)
    info.setSetting(value: frontendURL?.absoluteString, for: .permissionGatewayFrontendURL)
