...
final class CarPlayAssistSession: NSObject {
    ...
    private lazy var template: CPVoiceControlTemplate = {
        ...
        setActionButtons(actionButtons, on: idleState)
        ...
        setActionButtons(actionButtons, on: errorState)
    ...
    private func setActionButtons(_ actionButtons: [CPButton], on state: CPVoiceControlState) {
        let selector = NSSelectorFromString("setActionButtons:")
        guard state.responds(to: selector) else { return }
        state.setValue(actionButtons, forKey: "actionButtons")
    }
