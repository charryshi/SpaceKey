...
public class TokenManager {
    ...
    public func authDictionaryForWebView(forceRefresh: Bool) -> Promise<[String: Any]> {
        firstly { () -> Promise<(String, Date)> in
        ...
        }.map { [server] token, expiration -> [String: Any] in
            ...
            if let devicePublicKey = server.info.token.gatewayDevicePublicKey {
                dictionary["device_public_key"] = devicePublicKey
            }
            return dictionary
        }
    }
...
extension TokenManager: Authenticator {
    ...
    public func apply(_ credential: TokenInfo, to urlRequest: inout URLRequest) {
        ...
        if let devicePublicKey = credential.gatewayDevicePublicKey {
            urlRequest.headers.add(name: "X-Device-Public-Key", value: devicePublicKey)
        }
    }
    ...
    public func isRequest(_ urlRequest: URLRequest, authenticatedWith credential: TokenInfo) -> Bool {
        ...
        let hasBearerToken = urlRequest.headers["Authorization"] == bearerToken
        let hasDeviceBinding = credential.gatewayDevicePublicKey.map {
            urlRequest.headers["X-Device-Public-Key"] == $0
        } ?? true
        return hasBearerToken && hasDeviceBinding
    }
}
