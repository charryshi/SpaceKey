...
public class AuthenticationAPI {
    ...
    public func refreshTokenWith(tokenInfo: TokenInfo) -> Promise<TokenInfo> {
        if let gatewayDevicePublicKey = tokenInfo.gatewayDevicePublicKey {
            return refreshGatewayTokenWith(tokenInfo: tokenInfo, devicePublicKey: gatewayDevicePublicKey)
        }
    ...
    public func revokeToken(tokenInfo: TokenInfo) -> Promise<Bool> {
        if let gatewayDevicePublicKey = tokenInfo.gatewayDevicePublicKey {
            return revokeGatewayToken(tokenInfo: tokenInfo, devicePublicKey: gatewayDevicePublicKey)
        }
    ...
    private func refreshGatewayTokenWith(tokenInfo: TokenInfo, devicePublicKey: String) -> Promise<TokenInfo> {
        Promise { seal in
            guard let activeUrl = server.info.connection.activeURL() else {
                seal.reject(ServerConnectionError.noActiveURL(server.info.name))
                return
            ...
            let request = session.request(
                gatewayURL(activeUrl, pathComponents: ["v1", "auth", "refresh"]),
                method: .post,
                parameters: [
    ...
    private func revokeGatewayToken(tokenInfo: TokenInfo, devicePublicKey: String) -> Promise<Bool> {
        Promise { seal in
            guard let activeUrl = server.info.connection.activeURL() else {
                seal.reject(ServerConnectionError.noActiveURL(server.info.name))
                return
            ...
            let headers = HTTPHeaders([
                .authorization(bearerToken: tokenInfo.accessToken),
                .init(name: "X-Device-Public-Key", value: devicePublicKey),
            ])
            let request = session.request(
                gatewayURL(activeUrl, pathComponents: ["v1", "auth", "revoke"]),
                method: .post,
    ...
    private func gatewayURL(_ baseURL: URL, pathComponents: [String]) -> URL {
