...
public enum AppConstants {
    /// This fork is locked to the permission gateway flow. Native HA-only integrations that can
    /// bypass the gateway or request unrelated capabilities should stay disabled.
    public static let isPermissionGatewayBuild = true
    ...
    public static var AppGroupContainer: URL {
        ...
        if let groupDir = fileManager.containerURL(forSecurityApplicationGroupIdentifier: AppConstants.AppGroupID) {
            return groupDir
        }

        let applicationSupportDirectory = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask)
            .first ?? fileManager.temporaryDirectory
        let fallbackDirectory = applicationSupportDirectory.appendingPathComponent("AppGroupFallback", isDirectory: true)
        ...
        if !fileManager.fileExists(atPath: fallbackDirectory.path) {
            do {
                ...
                fatalError("Unable to create fallback App Group directory: \(error).")
            }
        }
