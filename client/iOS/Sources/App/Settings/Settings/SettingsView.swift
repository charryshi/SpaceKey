...
struct SettingsView: View {
    ...
    private var permissionGatewayView: some View {
        NavigationView {
            List {
                Section {
                    Label {
                        VStack(alignment: .leading, spacing: DesignSystem.Spaces.one) {
                            Text("授权网关")
                                .font(.headline)
                            Text("当前应用只通过授权网关访问设备。调试、通知、位置、传感器、快捷指令和多服务器管理入口已在此定制版中关闭。")
                                .font(.subheadline)
                    ...
                    Text("如需变更授权范围、延期或吊销 key，请在网关管理台操作。")
                }
            }
