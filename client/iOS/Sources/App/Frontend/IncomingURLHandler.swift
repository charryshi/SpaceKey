...
class IncomingURLHandler {
    ...
    enum IncomingURLAction: String {
        ...
        var isAllowedInPermissionGatewayBuild: Bool {
            ...
            case .navigate:
                return true
            case .xCallbackURL, .callService, .fireEvent, .sendLocation, .performAction, .assist, .invite,
                 .createCustomWidget, .camera:
                return false
            }
        }
    }
    ...
    func handle(url: URL) -> Bool {
        ...
        if let requestedAction = IncomingURLAction(rawValue: host.lowercased()) {
            if AppConstants.isPermissionGatewayBuild, !requestedAction.isAllowedInPermissionGatewayBuild {
                Current.Log.verbose("Ignoring URL action in permission gateway build: \(requestedAction)")
                return true
            }
    ...
    func handle(userActivity: NSUserActivity) -> Bool {
        ...
        guard !AppConstants.isPermissionGatewayBuild else {
            Current.Log.verbose("Ignoring user activity in permission gateway build")
            return false
        }
    ...
    func handle(shortcutItem: UIApplicationShortcutItem) -> Promise<Void> {
        guard !AppConstants.isPermissionGatewayBuild else {
            Current.Log.verbose("Ignoring app shortcut in permission gateway build")
            return .value(())
        }

        return Current.backgroundTask(withName: BackgroundTask.shortcutItem.rawValue) { remaining -> Promise<Void> in
