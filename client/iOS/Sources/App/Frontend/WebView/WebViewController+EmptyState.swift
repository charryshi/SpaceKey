...
extension WebViewController {
    func setupEmptyState() {
        let emptyState = WebViewEmptyStateWrapperView(
            ...
            settingsAction: AppConstants.isPermissionGatewayBuild ? nil : { [weak self] in
                self?.showSettingsViewController()
            },
            errorDetailsAction: AppConstants.isPermissionGatewayBuild ? nil : { [weak self] in
    ...
    func emptyStateStyle(for connectionState: FrontEndConnectionState) -> WebViewEmptyStateStyle {
        if AppConstants.isPermissionGatewayBuild {
            return .permissionGatewayDisconnected
        }
    ...
    var shouldShowErrorDetailsButton: Bool {
        guard !AppConstants.isPermissionGatewayBuild else { return false }
        return connectionState == .disconnected && latestLoadError != nil
    }
