...
enum WebViewExternalBusMessage: String, CaseIterable {
    ...
    @MainActor static var configResult: [String: Any] {
        if AppConstants.isPermissionGatewayBuild {
            return [
                ...
                "hasAssist": false,
                "hasAssistSettings": false,
                "hasCameraPlayer": false,
                "canSetupImprov": false,
                "downloadFileSupported": false,
                "hasEntityAddTo": false,
                "appVersion": "\(AppConstants.version) (\(AppConstants.build))",
                "toastComponentVersion": -1,
            ]
        }
    ...
    var isAllowedInPermissionGatewayBuild: Bool {
        switch self {
        case .configGet, .haptic, .connectionStatus, .themeUpdate, .focusElement, .toastShow, .toastHide:
            true
        case .configScreenShow, .tagRead, .tagWrite, .matterCommission, .threadImportCredentials,
             .threadStoreCredentialInAppleKeychain, .barCodeScanner, .barCodeScannerClose, .barCodeScannerNotify,
             .assistShow, .assistSettings, .scanForImprov, .improvConfigureDevice, .entityAddToGetActions,
             .entityAddTo, .cameraPlayerShow:
            false
        }
    }
}
