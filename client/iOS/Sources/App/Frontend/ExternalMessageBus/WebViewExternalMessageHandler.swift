...
final class WebViewExternalMessageHandler: @preconcurrency WebViewExternalMessageHandlerProtocol {
    ...
    func handleExternalMessage(_ dictionary: [String: Any]) {
        ...
        if let externalBusMessage = WebViewExternalBusMessage(rawValue: incomingMessage.MessageType) {
            if AppConstants.isPermissionGatewayBuild, !externalBusMessage.isAllowedInPermissionGatewayBuild {
                Current.Log.verbose("Blocked native external bus message in permission gateway build: \(externalBusMessage)")
                if let messageID = incomingMessage.ID {
                    response = .value(WebSocketMessage(
                        id: messageID,
                        type: "result",
                        result: [
                            "code": "permission_gateway_native_feature_disabled",
                            "message": "This native feature is disabled in the permission gateway build.",
                        ],
                        success: false
                    ))
                }
