...
struct OnboardingServersListView: View {
    enum Constants {
        ...
        enum MacSheetSize {
            ...
            static let gatewayActivationMinWidth: CGFloat = 720
            static let gatewayActivationMinHeight: CGFloat = 680
        }
    }
    ...
    @State private var showGatewayActivation = false
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showGatewayActivation) {
            PermissionGatewayActivationView { server in
                viewModel.gatewayActivationSucceeded(server: server)
            }
            .macOnboardingSheetFrame(
                minWidth: Constants.MacSheetSize.gatewayActivationMinWidth,
                minHeight: Constants.MacSheetSize.gatewayActivationMinHeight
            )
        }
    ...
    private var gatewayActivationButton: some View {
        Button(action: {
            showGatewayActivation = true
        }) {
            Label("Permission Gateway", systemImage: "qrcode.viewfinder")
        }
        .buttonStyle(.primaryButton)
    }
