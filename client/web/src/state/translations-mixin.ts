...
import { isPermissionGatewaySession } from "../data/permission_gateway";
  ...
  class extends superClass {
    ...
    protected hassConnected() {
      ...
      subscribeTranslationPreferences(this.hass!, async ({ value }) => {
        if (isPermissionGatewaySession(this.hass!.auth)) {
          const localLanguage = getLocalLanguage();
          if (this.hass!.language !== localLanguage) {
            this._selectLanguage(localLanguage, false);
          }
          return;
        }
    ...
    private _selectLanguage(language: string, saveToBackend: boolean) {
      ...
      if (saveToBackend && !isPermissionGatewaySession(this.hass.auth)) {
        saveTranslationPreferences(this.hass, this.hass.locale);
      }
      this._applyTranslations(this.hass);
