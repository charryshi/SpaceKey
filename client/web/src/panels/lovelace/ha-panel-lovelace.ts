...
export class LovelacePanel extends LitElement {
  ...
  private _permissionGatewayDashboardEntitySignature?: string;
  ...
  protected updated(changedProperties: PropertyValues<this>): void {
    ...
    if (
      oldHass &&
      this.hass &&
      isPermissionGatewaySession(this.hass.auth) &&
      shouldRestrictPermissionGateway(
        this.hass.permissionSummary,
        this.hass.auth
      ) &&
      !this._loading &&
      (oldHass.permissionSummary !== this.hass.permissionSummary ||
        this._permissionGatewayEntitySignature() !==
          this._permissionGatewayDashboardEntitySignature ||
        !this.lovelace ||
  ...
  private _registriesChanged = async () => {
    ...
    if (
      !isStrategyDashboard(rawConfig) ||
      !this._canGenerateDashboardStrategy()
    ) {
      return;
    }
  ...
  private async _regenerateStrategyConfig() {
    ...
    try {
      if (!this._canGenerateDashboardStrategy()) {
        return;
      }
  ...
  private async _fetchConfig(forceDiskRefresh: boolean) {
    ...
    if (
      ...
      const permissionGatewayConfig = createPermissionGatewayLovelaceConfig(
        this.hass?.states,
        this.hass?.permissionSummary,
        this.hass?.localize,
        this.hass?.language
    ...
    try {
      ...
      if (permissionGatewayConfig) {
        conf = permissionGatewayConfig;
        rawConf = permissionGatewayConfig;
      } else if (
        isPermissionGatewaySession(this.hass?.auth) &&
        (!this.hass?.permissionSummary ||
          (shouldRestrictPermissionGateway(
      ...
      else if (isStrategyDashboard(rawConf)) {
        if (!this._canGenerateDashboardStrategy()) {
      ...
      conf = filterLovelaceConfigByPermission(
        conf,
        this.hass?.permissionSummary
      );
      rawConf = filterLovelaceConfigByPermission(
        rawConf,
        this.hass?.permissionSummary
      );
    } catch (err: any) {
  ...
  private _permissionGatewayEntitySignature() {
    if (!this.hass?.states) {
      return undefined;
    }
    return Object.keys(this.hass.states).sort().join("|");
  }

  private _canGenerateDashboardStrategy() {
    return Boolean(
      this.hass?.config &&
      this.hass.states &&
      this.hass.entities &&
