...
import {
  createPermissionGatewayFallbackLovelaceConfig,
...
export const generateLovelaceDashboardStrategy = async (
  ...
  const restricts = shouldRestrictPermissionGateway(
    hass.permissionSummary,
    hass.auth
  );
  if (restricts) {
    return (
      createPermissionGatewayLovelaceConfig(
        ...
        hass.localize,
        hass.language
      ) ?? createPermissionGatewayFallbackLovelaceConfig(hass.permissionSummary)
    );
  }
...
export const generateLovelaceSectionStrategy = async (
  ...
  if (shouldRestrictPermissionGateway(hass.permissionSummary, hass.auth)) {
    return {
      type: "grid",
      cards: [],
    };
