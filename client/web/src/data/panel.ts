...
const CHINESE_PANEL_TITLES: Record<string, string> = {
  light: "灯",
  climate: "空调",
  security: "安防",
  maintenance: "维护",
...
export const localizePanelTitle = (
  localize: LocalizeFunc,
  language: string | undefined,
  panelTitle: string | undefined | null,
  ...
  if (isChineseLanguage(language) && CHINESE_PANEL_TITLES[panelTitle]) {
    return CHINESE_PANEL_TITLES[panelTitle];
  }

  return (
    localize(`panel.${panelTitle}` as LocalizeKeys) ||
    fallback ||
    panelTitle ||
    undefined
  );
...
export const getPanelTitle = (
  ...
  const panelTitle = [PROFILE_PANEL, NOT_FOUND_PANEL].includes(panel.url_path)
    ? panel.url_path
    : panel.title;

  return localizePanelTitle(
    hass.localize,
    hass.language,
    panelTitle,
    panel.title
