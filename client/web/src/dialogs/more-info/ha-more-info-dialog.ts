...
export class MoreInfoDialog extends SubscribeMixin(
  ...
  protected render() {
    ...
    const deviceId = this._getDeviceId();
    const deviceType = (deviceId && devices[deviceId]?.entry_type) || "device";
    ...
    const context = stateObj
      ? getEntityContext(stateObj, entities, devices, areas, floors)
      : this._entry
        ? getEntityEntryContext(this._entry, entities, devices, areas, floors)
        : undefined;

    const entityName = stateObj
      ? computeEntityName(stateObj, entities, devices)
      : this._entry
        ? computeEntityEntryName(this._entry, devices)
        : entityId;
