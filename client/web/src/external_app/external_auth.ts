...
import {
  ...
  markPermissionGatewayExternalAuthSession,
...
export class ExternalAuth extends Auth {
  ...
  constructor(hassUrl: string) {
    const gatewaySession = isPermissionGatewayExternalAuthSession();
    if (gatewaySession) {
      markPermissionGatewayExternalAuthSession();
    }
    super({
      hassUrl,
      clientId: gatewaySession ? PERMISSION_GATEWAY_CLIENT_ID : "",
      refresh_token: "",
      access_token: "",
      expires_in: 0,
  ...
  public async refreshAccessToken(force?: boolean) {
    ...
    this.data.expires_in = tokens.expires_in;
    this.data.expires = tokens.expires_in * 1000 + Date.now();
    savePermissionGatewayDevicePublicKey(tokens.device_public_key);
    this._tokenCallbackPromise = undefined;
  }
